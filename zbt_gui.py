...
class ZBTwindow(tk.Tk):
    ...
    def __init__(self, name, rows=1, columns=1, x_dim=500, y_dim=400):
        ...
        self.footnote = 'Ver.: ' + str(version) + ' (' + str(year) + ') by ' + author
        self.foot = tk.Label(self.bot, text=self.footnote, bg='grey25', fg='snow')
        self.foot.grid(row=0, column=0, sticky='e')

    # def ButtonEvent():
    #     buttonevent = ZBTtoplevel('POL-Analysis')

    def addButton(self, row, column, id, name, text='unnamed'):
        id = ZBTbutton(zbt.top, text=text, command=lambda: print('test'))
...
zbt = ZBTwindow('Data Analysis', rows=10, columns=3)

zbt.addButton(0, 1, id='b1', text='POL', name='Pol-Analysis')
# zbt.addButton(1, 1, id='b2', text='EIS')
# zbt.addButton(2, 1, id='b3', text='MS')
# zbt.addButton(3, 1, id='b4', text='ECR')
...
label_ph_pol = tk.Label(zbt.top, image=ph_pol, width=50, height=50, padx=10, pady=10)
label_ph_eis = tk.Label(zbt.top, image=ph_eis, width=50, height=50, padx=10, pady=10)
label_ph_ms = tk.Label(zbt.top, image=ph_ms, width=50, height=50, padx=10, pady=10)
label_ph_ecr = tk.Label(zbt.top, image=ph_ecr, width=50, height=50, padx=10, pady=10)
