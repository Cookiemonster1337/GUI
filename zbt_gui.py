...
class ZBTtoplevel(tk.Toplevel):
    ...
    def __init__(self, name, rows, columns, x_dim=1000, y_dim=800, *args, **kwargs):
        ...
        self.sub_top.grid(row=0, column=0, sticky='nesw')
        self.sub_bot.grid(row=1, column=0, sticky='nesw')
...
def buttonevent(subwindow):
    buttonevent = ZBTtoplevel(name=subwindow, rows=20, columns=6)

    sub_b1 = ZBTbutton(master=buttonevent.sub_top, text='Import', command=lambda: get_file())
...
def get_file():
    filename = \
        tk.filedialog.askopenfilename(initialdir="C:/Users/inrel/Desktop/exp_data", title="Select file",
...
b1 = ZBTbutton(master=main.top, text='POL', command=lambda: buttonevent('POL-Analysis'))
